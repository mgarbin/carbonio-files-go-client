...
- The SQLite cache file (`file_sync_cache.db`) is created automatically under `<home>/.carbonio_files_sync/` when using cache-based sync operations.
...
- The desktop GUI's credential database (`gui-config.db`, also under `<home>/.carbonio_files_sync/`) and its `.key` sibling are as sensitive as the CLI's `file_sync_cache.db`/`.key` and are never written to the project directory.
