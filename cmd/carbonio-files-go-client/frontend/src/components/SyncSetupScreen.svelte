  ...
  function complete() {
    ...
    api
      ...
      .then(() => {
        ...
        // Kick off the first cache scan automatically: the sqlite sync
        // cache needs to reflect the current local/remote state before any
        // sync operation can run. This can take a while (full local +
        // remote tree walk), hence the dedicated loading screen below.
        scanning = true;
        return api.updateCacheSync();
      })
      .then(() => {
        scanning = false;
...
{#if scanning}
  <Loader message={t("syncSetup.initialScanning")} />
{:else}
